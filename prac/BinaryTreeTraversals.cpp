 ...
 *
 * 该算法需要在节点信息里额外维护一个域
 * s，表示遍历过程中的访问次数，有如下几种取值： 0: 尚未访问 1:
 * 访问过1次(preorder)，尚未遍历其左右子树 2:
 * 访问过2次(inorder)，已遍历其左子树，尚未遍历其右子树 3:
 * 访问过3次(postorder)，已遍历其左右子树
 *
 * 访问到 x
 * 时，其祖先节点及它们的子树遍历进度(在遍历左子树还是右子树)是通过栈维护的。
 * 该算法把祖先节点下溯的指针翻转，再通过设置一个父节点指针 p
 * 以把祖先节点组织为一个链。 通过 s
 * 可以知道各祖先节点当前在遍历左子树还是右子树。
 *
 * 若注释掉代码中的 x->s = 0; 则在算法执行完后遍历到的节点的 s
 * 值为3，可以判断节点是否被访问过。
 *
 * 该算法可用于遍历一般的有向图，也需要注释掉 x->s = 0。带有 q
 * 个指针域的节点将被访问 q+1 次。
 */
...
// Schorr-Waite的变体，对于二叉树，上面 schorr_waite_alternative
// 的代码可以进一步简化 考察条件 p->s == 3 || p->l && p->l->s == 0 若 p->s != 3
// 则有 1 <= p->s && p->s <= 2 此时 p->l
// 为原节点的左子树或右子树，因为是树，不存在其他节点的指向它们 因此 p->l 若非
// NULL 则必有 p->l->s != 0 该条件只有在 p->l == NULL
// 时不成立。即使不成立，我们依然可以套用四指针置换 但需要把 p(此时为NULL) 设为
// q，把 q 设为 NULL 之后 s 仅用于判断节点访问次数，不再参与逻辑控制
